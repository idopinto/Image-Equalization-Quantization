...
def toy_test():
    ...
    plt.show()
    im_eq, hist_orig, hist_eq = s.histogram_equalize(grad / 255.)
    plt.imsave('toy_eq.jpg', im_eq, cmap='gray')
...
def equalize_my_profile_pic():
    s.imdisplay('RGB\WhatsApp-Image-2022-11-02-at-15.20.57.jpg', 2)
    im = s.read_image('RGB\WhatsApp-Image-2022-11-02-at-15.20.57.jpg', 1)
    print(im)
    im_eq, hist_orig, hist_eq = s.histogram_equalize(s.read_image('RGB\WhatsApp-Image-2022-11-02-at-15.20.57.jpg', 2))
    plt.imshow(im_eq, cmap=plt.cm.gray)
    plt.show()
...
def main():
    ...
    # Test 7 - histogram equalize on my profile pic
    # equalize_my_profile_pic()

    # Test 8 - histogram quantization grayscale
    # im = s.read_image('RGB/MyProfilePicture.jpg', 1)
    # im_quant, error = s.quantize(im, 6, 20)
    # display_im_and_im_qu(im, im_quant, 6)

    # Test 9 - histogram quantization RGB
